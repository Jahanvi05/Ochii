...
const Footer = () => {
  return (
    ...
    className='w-full h-full bg-black pt-28 pb-10 px-10 rounded-tl-3xl rounded-tr-3xl font-["FoundersGrotesk Condensed"]  flex'>
        <div className='flex-col h-full w-1/2'>
            <div className='text-white text-[15vh] font-semibold -tracking-[1.5vh]'>
            <h1 className='-mb-14 '>EYE-</h1>
            <h1>OPENING</h1>
            </div>

           
		<div className='logo px-1 mt-52'>
       <svg width="72" height="30" viewBox="0 0 72 30" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className='h-full w-1/2'>
            <div>
                <h1 className='uppercase text-white text-[15vh] font-semibold -tracking-[1.1vh]'>presentation</h1>
            </div>
            <div className='flex justify-between h-full'>
            <div className='flex-col'>
                <h1> S :</h1>
                <div className='links flex flex-col justify-content font-["Neue Montreal"] mt-4 font-light'>
                    {["Instagram","LinkedIn","Facebook"].map((item,index)=>(
                        <a className='underline underline-offset-4 decoration-[.1vh]' key={index} href="">{item}</a>
                    ))}
                </div>
                <h1 className='mt-11'> L :</h1>
                <div className='links flex flex-col justify-content font-["Neue Montreal"] mt-4 font-light'>
                    {["202-1965 W 4th Ave","Vancouver, Canada"].map((item,index)=>(
                        <a className='underline underline-offset-4 decoration-[.1vh]' key={index} href="">{item}</a>
                    ))}
                </div>
                <h1 className='mt-12 text-slate-600 opacity-2'>© ochi design 2024. Legal Terms</h1>
            </div>
            
            <div className='h-full mt-[26vh]'>
            <h1> S :</h1>
                <div className='links flex flex-col justify-content font-["Neue Montreal"] mt-4 '>
                    {["Home","Service","Facebook"].map((item,index)=>(
                        <a className='underline underline-offset-4 decoration-[.1vh]' key={index} href="">{item}</a>
                    ))}
                </div> 
            </div>
